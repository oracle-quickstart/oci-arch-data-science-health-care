...
resource "oci_core_network_security_group" "ADWSecurityGroup" {
  compartment_id = var.compartment_ocid
  display_name   = "ADWSecurityGroup"
  vcn_id         = oci_core_vcn.datas_vcn.id
  defined_tags   = { "${oci_identity_tag_namespace.ArchitectureCenterTagNamespace.name}.${oci_identity_tag.ArchitectureCenterTag.name}" = var.release }
}

# Rules related to ADWSecurityGroup

# EGRESS

resource "oci_core_network_security_group_security_rule" "ADWSecurityEgressGroupRule" {
  network_security_group_id = oci_core_network_security_group.ADWSecurityGroup.id
  direction                 = "EGRESS"
  protocol                  = "6"
...
resource "oci_core_network_security_group_security_rule" "ADWSecurityIngressGroupRules" {
  network_security_group_id = oci_core_network_security_group.ADWSecurityGroup.id
  direction                 = "INGRESS"
  protocol                  = "6"
  source                    = "0.0.0.0/0"
...
resource "oci_core_network_security_group_security_rule" "APPSecurityEgressADWGroupRule" {
  network_security_group_id = oci_core_network_security_group.APPSecurityGroup.id
  direction                 = "EGRESS"
  protocol                  = "6"
  destination               = oci_core_network_security_group.ADWSecurityGroup.id
